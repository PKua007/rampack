...
#include "core/dynamic_parameters/ShiftedDynamicParameter.h"
...
TEST_CASE("ShiftedParameterUpdater: positive shift") {
    SECTION("correct (non-negative cycles)") {
        auto mockUpdater = std::make_unique<MockDynamicParameter>();
        REQUIRE_CALL(*mockUpdater, getValueForCycle(100ul, 800ul)).RETURN(5);
        ShiftedDynamicParameter shiftedParameterUpdater(200, std::move(mockUpdater));
    ...
    SECTION("incorrect (negative cycles)") {
        ...
        ShiftedDynamicParameter shiftedParameterUpdater(1000, std::move(mockUpdater));
