...
TEST_CASE("PolyhedralWedgeTraits: geometry") {
    ...
    ShapeData data = traits.shapeDataForSpecies(3, 1, 2, 4, 6);
...
TEST_CASE("PolyhedralWedgeTraits: overlap") {
    ...
    DYNAMIC_SECTION("subdivisions: " << subdivisions) {
        PolyhedralWedgeTraits traits;
        ShapeData data = traits.shapeDataForSpecies(3, 1, 2, 4, 6, subdivisions);
        const auto &interaction = traits.getInteraction();
        FreeBoundaryConditions fbc;
        Shape shape0({}, Matrix<3, 3>::identity(), data);

        // This case was visually inspected using Mathematica
        SECTION("vertex-face") {
            auto rot1 = Matrix<3, 3>::rotation(0, 1, 0.5);
            Shape shapeOv1({-2.92, -2.92, 2.92}, rot1, data);
            Shape shapeNonOv1({-2.93, -2.93, 2.93}, rot1, data);
        ...
        SECTION("face-face") {
            ...
            Shape shapeOv2({0, 0, 5.99}, rot2, data);
            Shape shapeNonOv2({0, 0, 6.01}, rot2, data);
