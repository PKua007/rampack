                                                               ...
                                                               std::mt19937 &mt) const
{
    ...
    deltaDim.fill((2 * unitIntervalDistribution(mt) - 1) * scalingStepSize);
    switch (this->scalingDirection) {
        case ScalingDirection::ISOTROPIC:
            break;
        case ScalingDirection::ANISOTROPIC_X:
            deltaDim[0] = (2 * unitIntervalDistribution(mt) - 1) * scalingStepSize;
            break;
        case ScalingDirection::ANISOTROPIC_Y:
            deltaDim[1] = (2 * unitIntervalDistribution(mt) - 1) * scalingStepSize;
            break;
        case ScalingDirection::ANISOTROPIC_Z:
            deltaDim[2] = (2 * unitIntervalDistribution(mt) - 1) * scalingStepSize;
            break;
