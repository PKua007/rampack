...
std::vector<Shape> RandomPopulator::populateLattice(const Lattice &lattice, std::optional<std::size_t> numOfShapes) const {
    std::size_t latticeSize = lattice.size();
    if (!numOfShapes.has_value() || *numOfShapes == latticeSize)
