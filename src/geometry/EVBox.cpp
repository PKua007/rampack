...
EVBox::EVBox(const Vector<3> &v, double l)
        : position{v}, length{l}
{
...
void EVBox::divide(std::vector<EVBox *> &newBoxes) const{
    ...
    for(unsigned int ix = 0; ix < 2; ix++) {
        for (unsigned short iy = 0; iy < 2; iy++) {
            for (unsigned short iz = 0; iz < 2; iz++) {
...
void EVBox::sampleMC(Matrix<3, 3, double> *orientation, const Shape& originShape, const Shape& testShape, const Interaction &interaction, size_t samples){
    size_t threads = OMP_MAXTHREADS;
    std::size_t *counter = new size_t[threads];
    ...
    #pragma omp parallel for default(none) shared(samples, testShapes, orientation, interaction, originShape, counter) \
            schedule(dynamic)
